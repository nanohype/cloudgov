...
- [ ] **Thread `--quiet` to provider warnings**: the 18 unconditional `os.Stderr` warn spots across `internal/cloud/aws/{iam,cloudtrail,quota,orphans}.go` ignore `--quiet`. Add a provider-level quiet signal (constructor option → a `p.warnf` helper) threaded via a `providers.WithQuiet` option through the 12 resolvers.
