...
//
// A handler that reads saved reports resolves nothing, so package cmd drives it
// directly — its flags, its exit code and the artifact it writes are exercised
// there rather than read. A handler that reaches an account resolves providers
// through providers.Resolve (→ Default()), which is AWS-backed and has no
// test-injection seam, so nothing runs those: package cmd tests the pure helpers
// they call and checks their SOURCE for invariants an AST can see, which proves
// a call site exists rather than that the value reaches it.
//
