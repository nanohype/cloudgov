...
func cisAWSv3Benchmark() *Benchmark {
	return &Benchmark{
		...
		Controls: []Control{
			...
			{ID: "1.4", Title: "Ensure no root account access key exists", Section: "Identity and Access Management", Severity: cloud.SeverityCritical, Description: "The root account should not have access keys. cloudgov does not read root credential state, so this control is reported as not evaluated."},
			{ID: "1.5", Title: "Ensure MFA is enabled for the root account", Section: "Identity and Access Management", Severity: cloud.SeverityCritical, Description: "The root account should have MFA enabled. cloudgov does not read root credential state, so this control is reported as not evaluated."},
			...
			{ID: "1.17", Title: "Ensure a support role has been created for incident management", Section: "Identity and Access Management", Severity: cloud.SeverityLow, Description: "A support role should exist. cloudgov reads the principals it finds rather than checking for one it expects, so this control is reported as not evaluated."},
			{ID: "1.19", Title: "Ensure IAM instance roles are used for resource access", Section: "Identity and Access Management", Severity: cloud.SeverityMedium, Description: "EC2 instances should use IAM roles for API access. cloudgov does not enumerate instance-to-role attachment, so this control is reported as not evaluated."},
			{ID: "1.22", Title: "Ensure access to AWSCloudShellFullAccess is restricted", Section: "Identity and Access Management", Severity: cloud.SeverityMedium, Description: "CloudShell full access should be restricted. cloudgov classifies permissions rather than tracking named managed policies, so this control is reported as not evaluated."},
			...
			// 3 - Logging
			{ID: "3.1", Title: "Ensure CloudTrail is enabled in all regions", Section: "Logging", Severity: cloud.SeverityHigh, Description: "CloudTrail should be enabled across all regions. cloudgov reads CloudTrail events rather than the trails that produce them, so this control is reported as not evaluated."},
			{ID: "3.4", Title: "Ensure CloudTrail log file validation is enabled", Section: "Logging", Severity: cloud.SeverityMedium, Description: "CloudTrail should have log file validation enabled. cloudgov does not read trail configuration, so this control is reported as not evaluated."},
			...
			{ID: "5.4", Title: "Ensure the default security group of every VPC restricts all traffic", Section: "Networking", Severity: cloud.SeverityMedium, Description: "Default security groups should restrict all traffic. cloudgov does not read default security groups, and a rule permitting all traffic from the group itself is not collected, so this control is reported as not evaluated; network findings are context toward it, not a verdict on it."},
		},
	}
}
